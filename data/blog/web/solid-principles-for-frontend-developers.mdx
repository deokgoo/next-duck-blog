---
title: 프론트엔드 개발자를 위한 SOLID 원칙 - TypeScript와 React로 이해하기
summary: SOLID 원칙을 프론트엔드 개발자 관점에서 실용적으로 이해하고, TypeScript와 React 예제로 실무에 적용하는 방법을 알아봅니다.
tags: [SOLID, TypeScript, React, 객체지향, 설계 원칙, 클린 코드]
date: 2025-10-11
...
// 2. 검색 필터링 책임 - Custom Hook
function useProductSearch(products: Product[], searchTerm: string) {
  return useMemo(() => {
...
class AnalyticsService {
  ...
  static trackProductClick(productId: string) {
    this.trackEvent('product_click', { productId })
  }
}

// 5. UI 렌더링 책임만 남은 컴포넌트
function ProductList() {
  const [searchTerm, setSearchTerm] = useState('')
...
- **커스텀 훅으로 로직 분리**: 데이터 페칭, 검색/필터링, 폼 관리 등을 별도 훅으로 분리
- **서비스 레이어 구성**: API 호출, Analytics, 로컬 스토리지 등을 별도 서비스로 분리
- **유틸리티 함수 분리**: 포맷팅, 검증, 계산 로직 등을 순수 함수로 분리
- **프레젠테이셔널/컨테이너 패턴**: 비즈니스 로직과 UI를 명확히 분리
...
### React의 Composition Pattern

React의 컴포넌트 합성은 OCP를 자연스럽게 구현할 수 있는 강력한 패턴입니다.
...
- **플러그인 아키텍처**: 기능을 플러그인으로 확장 가능하게 설계
- **Render Props / Compound Components**: React의 합성 패턴 활용
- **설정 기반 렌더링**: JSON 설정으로 UI 동적 생성
...
- **서비스 레이어 분리**: API 호출, 저장소, 알림 등을 서비스로 추상화
- **Context로 의존성 주입**: Provider 패턴으로 구현체 주입
- **Custom Hook으로 캡슐화**: 구현 세부사항을 Hook 안에 숨기기
- **환경별 구현체 교체**: 개발/테스트/프로덕션 환경에 맞는 구현체 주입
