...
graph TD
    ...
    subgraph AFTER["SRP 준수"]
        G[ProductList Component] --> H["UI 렌더링"]
...
```mermaid
graph TD
    subgraph BEFORE["OCP 위반"]
        A[Notification Component]
        A --> B["Success UI"]
        A --> C["Error UI"]
        A --> D["Warning UI"]
        A -.-> A["새로운 타입 추가 시<br/>기존 코드 수정"]
    end

    subgraph AFTER["OCP 준수"]
        E[Notification Component] --> F["notificationConfigs"]
        F --> G["Success Config"]
        F --> H["Error Config"]
        F --> I["Warning Config"]
        J["New Config"] -.-> F["새로운 타입 추가<br/>설정만 추가"]
    end
```
...
```mermaid
graph TD
    subgraph LSP["LSP 위반 예시"]
        A[PaymentProcessor] --> B["PaymentMethod"]
        B --> C[CreditCardPayment]
        B --> D[KakaoPayPayment]
        C --> E["canProcess(): true"]
        D --> F["canProcess(): true/false<br/>(예외적 동작)"]
    end
```
